...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  await sharp(req.files.imageCover[0].buffer).resize(2000, 1333) // height and width needs to be the same for a square image!
  .toFormat('jpeg').jpeg({
    quality: 90
  }).toFile(`img/tours/${imageCoverFilename}`); // to store it in users folder in project!
  ...
  req.files.images.map(async (file, i) => {
    ...
    await sharp(file.buffer).resize(2000, 1333) // height and width needs to be the same for a square image!
    .toFormat('jpeg').jpeg({
      quality: 90
...
exports.getTourStats = catchAsync(async (req, res, next) => {
  ...
  // second stage is group:
  {
    $group: {
      ...
      // we want to calculate avarages for all groups, that's why is is null
      ...
      // to calculate the number of Tours
      numRatings: {
        $sum: '$ratingsQuantity'
      },
      avgRating: {
...
exports.getMonthlyPlan = catchAsync(async (req, res, next) => {
  ...
  // match stage is used to select a date here => startDate
  {
    ...
    $sort: {
      numTourStarts: -1
    } // -1 is for descending => starting with highest number
