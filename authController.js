...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
    ...
    // NOTE: when there is no token in authorization header => if part is false => then come here in else if: => jwt is the name of the cookie that we see it in Terminal in cookie content!
  } else if (req.cookies.jwt) {
    token = req.cookies.jwt; // now, we can authenticate a user based on token sent via cookie and not only authorization header in if part!
  }
  // console.log(token);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  console.log({
    randomResetToken: resetToken
  });
  await user.save({
    validateBeforeSave: false
  }); // this will deactivate all the validators
