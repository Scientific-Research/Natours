...
reviewSchema.statics.calcAverageRatings = async function (tourId) {
  ...
  // 1- select all the reviews which belong to the current tour which passed here as argument:
  {
    $match: {
      tour: tourId
    } // we select the tour which we want to update!
  }, {
    $group: {
      _id: '$tour',
      // we are grouping all the tours together by tour:
      nRating: {
        $sum: 1
      },
      // add one for the tour which matched from last step
      avgRating: {
        $avg: '$rating'
      } // calculate the average of rating numbers
    }
  }]);
  //  console.log(stats);
