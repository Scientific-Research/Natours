...
const tourSchema = new mongoose.Schema({
  ...
  startLocation: {
    ...
    coordinates: [Number],
    // this number is coordinates of the point: Langitude and Latitude.
    address: String,
    description: String
  },
  locations: [
  ...
  // when the data outputted as JSON
  toObject: {
    virtuals: true
  } // when the data outputted as Object
});
...
tourSchema.virtual('reviews', {
  ref: 'Review',
  // here we need the name of the model to that we reference!
  foreignField: 'tour',
  // tour is located in reviewModel.js
...
tourSchema.pre(/^find/, function (next) {
  this.find({
    ...
    secretTour: {
      $ne: true
    } // find the tours in which secretTour is not equal true.
...
tourSchema.pre(/^find/, function (next) {
  ...
  this.populate({
    path: 'guides',
    // we populate guides field that we just specified before!
